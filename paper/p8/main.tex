...
$500$-step window. We conclude that $\Tstar$ must be \emph{measured}
per deployment, not predicted from architecture, and that the APB is the
operational vehicle by which that measured threshold yields accountable
authority transfer.
\end{abstract}
...
\input{exp_a_table_default}

\input{exp_a_policy_comparison}
...
\paragraph{Result.}
\textbf{(Pending: full run in progress; table will be inserted from
\texttt{results/p8\_apb/exp\_d/exp\_d\_table.tex} when the experiment
completes.)}

\input{exp_d_table_placeholder}

\paragraph{Reading.}
\textbf{(To be written from results.)} Three different families and
three different baseline drift speeds; if the per-model range across
temperatures is indeed at most $10$ steps in all three cases, the
insensitivity claim generalises beyond the single-family Mistral
result of \citep{fernandez2026applied} and supports the framing of
$\Tstar$ as a deterministic, protocol-driven quantity.
...
across $3{,}812$ halt events, $1{,}800$ adversarial attempts, and
$18$ cross-model runs (with a fourth experiment on temperature
insensitivity completing as of writing) shows that the construction
behaves as the theory predicts and that the model-specific drift
threshold $\Tstar$ is stable, measurable, and architecture-driven
rather than scale-driven. The result is a small, configurable,
