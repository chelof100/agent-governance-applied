...
model, we replicate the detection task with a real LLM making decisions
from natural language context.
We use \textbf{mistral-small3.1} (15B parameters, temperature\,=\,0.4)
served locally via Ollama.

The LLM is given a task description that escalates in urgency and risk
over 150 drift steps following a 50-step burn-in.
During burn-in, the LLM receives routine maintenance scenarios;
...
Metric & Value \\
\midrule
Model                       & mistral-small3.1 (Ollama, T=0.4) \\
Drift steps                 & 150 \\
$\Dhat_{\mathrm{final}}$    & 0.367 \\
$T^*$ (steps after burn-in) & 64 \\
$\sum g(\tau)$              & 0 \\
T2 (enforcement blind)      & Pass \\
T3 (IML detects drift)      & Pass \\
\bottomrule
\end{tabular}
\caption{Experiment~1b: live LLM drift detection.
The enforcement signal is identically zero throughout;
IML detects drift at $T^*=64$ with $\Dhat_{\mathrm{final}}=0.367$.}
\label{tab:exp1b}
\end{table}
...
The result confirms the central claim of Experiment~1 under real-LLM
conditions.
The LLM makes a sequence of contextually appropriate decisions:
...
No enforcement rule is triggered ($g(\tau) = 0$ throughout).

Yet the IML composite $\Dhat$ grows from $0.007$ at the start of the drift
phase to $0.367$ at the end, crossing $\theta=0.20$ at step~$T^*=64$.
...
synthetic drift models.
A real LLM making contextually appropriate choices, responding to
task context in a way that any operator would approve step-by-step,
produces behavioral drift that is structurally invisible to enforcement
