...
Usage:
    ...
    python exp1b_multimodel.py --model deepseek-r1:8b --temperature 0.4

Output:
    results/exp1b/<model_slug>/summary.json
    results/exp1b/<model_slug>/results.json
"""
import argparse
import json
import os
...
def run_experiment(model: str, temperature: float) -> dict:
    ...
    print(divider)
    print(f"Running {TOTAL} steps (burn-in={BURN_IN}, drift={DRIFT})")
    print(f"Model: {label}")
    print("-" * 60)
...
def main():
    ...
    out_dir = os.path.join(_ROOT, "results", "exp1b", slug)
    os.makedirs(out_dir, exist_ok=True)

    result = run_experiment(args.model, args.temperature)
    s      = result["summary"]
